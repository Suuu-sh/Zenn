---
title: 'AuroraのStorage I/Oでコストが爆発した話と原因・対策'
emoji: '💥'
type: 'tech'
...
本番では `r5.2xlarge` を使っていたため、  
十分なメモリ量があり、  
**多くのデータがメモリ上にキャッシュされていました。**

一方 dev 環境では、

- インスタンスタイプが `t4g.medium`
- メモリが非常に少ない
- キャッシュに載りきらないデータが大量に存在
...
- クエリ自体は遅くない  
- アプリケーションのエラーも出ない  
- 「dev環境だから多少遅くても問題ない」と思いがち  

しかし裏では、  
**ストレージに対するI/Oだけが静かに増え続けていました。**
